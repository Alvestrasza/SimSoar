...
export default async function CompetitionPage({params}: {params: Promise<{locale: string; slug: string}>}) {
  ...
  const competition = await prisma.competition.findFirst({
    ...
    include: {entries: {
      orderBy: {score: "desc"},
      include: {user: {select: {profile: {select: {callsign: true}}}}, flight: {select: {id: true, title: true, simulator: true, competitionClass: true, startTime: true, createdAt: true, distanceKm: true, olcPoints: true, authenticitySubmissions: {orderBy: {revision: "desc"}, take: 1, select: {status: true, revision: true}}}}}
    }}
  });
  ...
  return <main className="wrap">
    <section className="card">
      ...
      {competition.evidenceRequired ? <div className="cardBody"><h3>{t("evidenceTitle")}</h3><p>{t("evidencePolicy", {signed: competition.requireSignedEvidence ? t("yes") : t("no")})}</p><p className="muted">{competition.requiredEvidenceFields.join(", ") || t("evidenceNoFields")}{competition.requiredTaskPackageId ? ` · ${competition.requiredTaskPackageId}` : ""}</p></div> : null}
    </section>
    ...
    <section className="card" style={{marginTop: 20}}><div className="cardHead"><span className="cardTitle">{t("flightsTitle")}</span></div><div className="tableWrap"><table>
      <thead><tr><th>{t("date")}</th><th>{t("pilot")}</th><th>{t("flight")}</th><th>{t("score")}</th><th>{t("evidence")}</th></tr></thead>
      <tbody>{competition.entries.length === 0 ? <tr><td colSpan={5} className="emptyTable">{t("noEntries")}</td></tr> : competition.entries.map((entry) => <tr key={entry.id}><td>{(entry.flight.startTime || entry.flight.createdAt).toLocaleDateString(locale)}</td><td>{entry.user.profile?.callsign || t("unnamedPilot")}</td><td><Link href={`/flights/${entry.flight.id}`}>{entry.flight.title}</Link></td><td>{entry.score.toFixed(1)}</td><td>{entry.flight.authenticitySubmissions[0]?.status ?? t("evidenceMissing")}</td></tr>)}</tbody>
    </table></div></section>
  </main>;
}

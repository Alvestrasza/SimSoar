...
import GroupFlightReplay from "@/app/components/GroupFlightReplay";
...
export default async function FlightComparisonPage({params, searchParams}: {params: Promise<{locale: string}>; searchParams: Promise<{id?: string | string[]}>}) {
  ...
  return <main className="wrap">
    ...
    {!ready ? <section className="card comparisonHint"><p>{t("selectionHint")}</p><p className="muted">{t("similarHint")}</p></section> : <>
      <GroupFlightReplay flights={flights.map((flight) => ({id: flight.id, title: flight.title, pilotCallsign: flight.pilotCallsign, track: flight.track.map((point) => ({seq: point.seq, lat: point.lat, lon: point.lon, altM: point.altM, time: point.time?.toISOString() ?? null}))}))} />

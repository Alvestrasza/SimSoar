...
| `MAX_SERVER_ACTION_BODY_SIZE` | `110mb` | Next.js request-body limit applied at build time to IGC and airspace uploads. |
| `MAX_IGC_BULK_BODY_SIZE` | unset | Backward-compatible fallback when `MAX_SERVER_ACTION_BODY_SIZE` is not set. |

The effective server action body limit must be large enough for the configured
combined file size plus multipart form overhead. Invalid, missing, or non-positive
